...
contract MetadataHandler is IMetadataHandler {
    ...
    function renderFNFT(address revest, bytes32 salt) internal view returns (string memory) {
        ...
        string memory image =
            '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 388"> <style> .tokens { font: bold 30px sans-serif; } .fee { font: bold 24px sans-serif; } .amount { font: bold 46px sans-serif ; fill: ';

        {
            ...
            image = string.concat(
                image,
                ';} .interest { font: bold 11px sans-serif; } .tick { font: normal 18px sans-serif; } .button { fill: #007bbf; pointer: cursor; } .button:hover { fill: #0069d9; } </style> <rect width="240" height="388" rx="15" ry="15 fill="hsl(0,0%,18%)" />'
            );
